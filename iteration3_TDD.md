...
|Different Devices|When the user install the application on a Samsung Galaxy S22, all of the expected functions would perform smoothly.|
|Different Devices|When the user install the application on a Xiaomi Pad 5, all of the expected functions would perform smoothly, and the longer vocabularies will be fully displayed due to larger screen.|
|Different Devices|When the user turn his mobile phone to landscape mode, the running application should automatically switch to the landscape mode with each elements on all interfaces in apporpriate proportions. The longer vocabularies will be fully displayed in the gameplay interface in each cell.| 
|Vocabulary Lists|When the user in the login interface, there should be two buttons, one displays login as teacher, another displays login as student.|
|Vocabulary Lists|When the user login as teacher, there should be a button in the welcome interface that directs the user to an activity to input words pairs and assign them to specific student account|
|Vocabulary Lists|When the user login as students, there should be a button in the welcome interface that directs the user to an activity to load words pairs that would be used in generating sudoku puzzle into his own account, then,  the user clicks the play button the gameplay interface, the newly loaded vocab would be shown in the sudoku puzzle|
|Vocabulary Lists|When the user unable to fill the French word for "car" in multiple rounds, the word will continuously be used in the following generated sudoku puzzle.|
